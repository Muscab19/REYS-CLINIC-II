...
const DoctorConsultation = () => {
      ...
      if (data.success && data.data.length > 0) {
        const testsWithRanges = await Promise.all(data.data.map(async (test) => {
          try {
            ...
            return { ...test, normalRange: null, unit: '' };
          }
        }));
  ...
  const getResultStatusBadge = (status) => {
    switch(status) {
      case 'normal':
        return <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-semibold">✓ Normal</span>;
      case 'abnormal':
        return <span className="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs font-semibold">⚠️ Abnormal</span>;
      default:
  ...
  try {
    const token = localStorage.getItem('token');
    // Use the doctor-specific endpoint (no prices)
  ...
  const handleStartConsultation = (patient) => {
    // Check if this is a follow-up patient
  ...
  try {
    ...
    // First, update patient as inpatient
  ...
  if (exists) {
    ...
    setSelectedMedications([...selectedMedications, { 
      ...
      isCustom: false,  // Mark as from inventory
      dosage: '',
      frequency: '',
      duration: '',
      route: '',
...
const handleCompleteConsultation = async () => {
  ...
  try {
    // Calculate total lab test amount
    const totalLabAmount = selectedLabTests.reduce((sum, test) => sum + (test.price || 0), 0);
    
    // Create lab payment record via API
    let labPaymentCreated = false;
    if (selectedLabTests.length > 0) {
      try {
        const token = localStorage.getItem('token');
        const labPaymentResponse = await fetch(`${API_BASE_URL}/api/lab-payments`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        const labPaymentData = await labPaymentResponse.json();
        if (labPaymentData.success) {
          labPaymentCreated = true;
          console.log('Lab payment record created:', labPaymentData.data);
        }
      } catch (error) {
        console.error('Error creating lab payment record:', error);
      }
    }
    ...
    const consultationRecord = {
      ...
      labPaymentStatus: labPaymentCreated ? 'pending_payment' : 'none'
    };
    
    // Save consultation record to localStorage for backward compatibility
    ...
    const statusUpdated = await updatePatientStatus(selectedPatient._id, 'pending-payment');
    ...
    if (selectedLabTests.length > 0) {
      toast.success(`Consultation completed! ${selectedLabTests.length} lab test(s) require payment at reception.`);
    } else {
  ...
  const sendLabTestRequests = async () => {
    try {
      ...
      if (successCount > 0) {
        toast.success(`${successCount} lab test request(s) sent to laboratory`);
      }
    } catch (error) {
      console.error('Error sending lab test requests:', error);
      toast.error('Failed to send lab test requests');
    }
  };
  ...
  try {
    const token = localStorage.getItem('token');
    // Filter out custom medications (those without a valid _id or with isCustom flag)
  ...
  try {
    ...
    if (data.success) {
      ...
      toast.error(data.msg || 'Failed to update patient status');
      return false;
    }
  } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-6xl w-full max-h-[85vh] flex flex-col shadow-2xl">
            <div className="p-4 border-b flex justify-between items-center sticky top-0 bg-white rounded-t-2xl z-10">
              <div>
                <h3 className="text-xl font-bold text-gray-900">Select Diagnoses</h3>
                <p className="text-xs text-gray-500 mt-0.5">Choose diagnoses for this consultation</p>
              </div>
            ...
            <div className="p-4 border-b bg-gray-50">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#D01A2B] focus:border-transparent text-sm"
                  autoFocus
                />
              </div>
            </div>
          ...
          {selectedMedications.map((med, idx) => (
            <tr key={med.id} className={`border-b ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
              <td className="p-2">
                ...
                {med.isCustom && <span className="ml-1 text-xs bg-yellow-200 text-yellow-800 px-1 rounded">Custom</span>}
              </td>
              <td className="p-2">
                <input 
                  type="text" 
                  ...
                  className="w-full p-1.5 border rounded text-sm focus:ring-2 focus:ring-[#D01A2B] focus:outline-none" 
                />
              </td>
              <td className="p-2">
                <input 
              ...
              <td className="p-2 text-center">
                <button onClick={() => handleRemoveMedication(med.id)} className="text-red-500 hover:text-red-700">
                  <X className="w-4 h-4" />
                </button>
              </td>
            </tr>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b p-4 flex justify-between">
              <h3 className="text-xl font-bold">Lab Results - {selectedPatient.childName}</h3>
              <button onClick={() => setShowLabResultsModal(false)}><X className="w-6 h-6" /></button>
            </div>
            <div className="p-6">
              <div className="bg-blue-50 rounded-xl p-4 mb-6">
                <div className="flex justify-between">
                  <div><p className="font-semibold">{selectedPatient.childName}</p><p className="text-sm">Age: {selectedPatient.childAge} years</p></div>
                  <div className="text-right"><p className="text-sm">Total Tests</p><p className="text-2xl font-bold text-[#D01A2B]">{labResults.length}</p></div>
                </div>
              </div>
                  ...
                  return (
                    <div key={idx} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start mb-3">
                        <div><p className="font-semibold text-lg text-[#D01A2B]">{test.testName}</p>{getResultStatusBadge(resultStatus)}</div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className={`p-3 rounded ${resultStatus === 'abnormal' ? 'bg-red-50' : 'bg-green-50'}`}>
                          <p className="text-xs">Result</p>
                          <p className={`text-xl font-bold ${resultStatus === 'abnormal' ? 'text-red-700' : 'text-green-700'}`}>{resultValue} {test.unit}</p>
                        </div>
                        <div className="p-3 rounded bg-gray-50"><p className="text-xs">Normal Range</p><p className="font-semibold">{test.normalRange || 'Not specified'}</p></div>
                      </div>
                    </div>
                  );
                })}
                ...
                <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-4 flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-yellow-600 mt-2">
                      Please review previous medical history and update accordingly.
                    </p>
                  </div>
                </div>
              </div>
  ...
  <button 
    onClick={handleOpenMedicationsModal} 
  ...
  >
    ...
    <span>{selectedMedications.length > 0 ? 'Edit Medications' : 'Select Medications'}</span>
  </button>
  
  {selectedMedications.length > 0 && (
    <div className="mt-3 space-y-2 max-h-48 overflow-y-auto">
      {selectedMedications.map(med => (
        <div key={med.id} className="p-2 bg-green-50 rounded text-sm">
          <div className="flex justify-between">
            <div className="flex-1">
              <span className="font-medium">{med.name}</span>
              {med.isCustom && <span className="ml-2 text-xs bg-yellow-200 text-yellow-800 px-1 rounded">Custom</span>}
              <div className="text-xs text-gray-600 mt-1">
                {med.dosage && <span>💊 {med.dosage}</span>}
                {med.frequency && <span className="ml-2">⏰ {med.frequency}</span>}
                {med.duration && <span className="ml-2">📅 {med.duration}</span>}
                {med.route && <span className="ml-2">💉 {med.route}</span>}
              </div>
              {med.instructions && <p className="text-xs text-gray-500 mt-1">📝 {med.instructions}</p>}
            </div>
            <button onClick={() => handleRemoveMedication(med.id)} className="text-red-500">
              <X className="w-3 h-3" />
            </button>
          </div>
        </div>
  ...
  {selectedMedications.length > 0 && (
    <button 
      ...
      className="mt-2 w-full py-1.5 bg-green-100 text-green-700 rounded-lg text-sm"
    >
      Print Prescription
    </button>
  )}
      ...
      {/* Past Consultations Modal */}
      {showPastConsultations && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[80vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b p-4 flex justify-between">
              <h3 className="text-xl font-bold">Past Consultations</h3>
              <button onClick={() => setShowPastConsultations(false)}><X className="w-6 h-6" /></button>
            </div>
            <div className="p-6">
              {pastConsultations.length === 0 ? (<div className="text-center py-8">No past consultations found</div>) : (
                <div className="space-y-4">
                  {pastConsultations.map((cons) => (<div key={cons.id} className="border rounded-lg p-4"><div className="flex justify-between"><p className="font-semibold text-[#D01A2B]">{cons.consultationId}</p><span className="text-sm text-gray-500">{new Date(cons.date).toLocaleString()}</span></div><div className="mt-2 text-sm"><span className="font-medium">Diagnoses:</span> {cons.diagnoses?.join(', ') || 'N/A'}</div><div className="text-sm"><span className="font-medium">Treatment:</span> {cons.treatment || 'N/A'}</div></div>))}
                </div>
              )}
            </div>
          </div>
